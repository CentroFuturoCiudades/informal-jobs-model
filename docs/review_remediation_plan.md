...
Status: **Phases 0–4 and all follow-ups done** (2026-08-28; 4.5 and 4.11 evaluated, not changed). Headline: OD expected informality **35.96%** (sampled 35.42%; raked to the ENOE metro profile ≈38.4%; ENOE metro benchmark 39.35%; no-trip sensitivity band 36.0–43.2%; components: informal sector 24.6% + unprotected 12.0%). Source: full-pipeline review of 2026-08-27 (after the mxcensus/eodgdl migration, commit `849568a`). Each item records *what is wrong*, *why it matters*, *the fix*, and *how to verify*.
...
### 4.8 OD-only mobility features for the sector model ✅ done
- **What:** mode of the work trip (`modo_trabajo`, from the trips), weekend work destination (`weekend_dest_trabajar`) and household vehicles (`n_autos_camionetas`, from `viv`), added to both sector specifications (`OD_MOBILITY_FEATURES`). Paired folds (selected with-education config): −0.0042 ± 0.0019 SE vs the destination-feature baseline — small but consistent. `modo_trabajo` is excluded from the auxiliary destination models (it is missing exactly when the destination is).
- **Result (2026-08-28):** shipped run: sector held-out log loss 0.840 [0.800, 0.877], accuracy 65.2%, macro-F1 0.598 (vs 0.839 / 64.8% / 0.592 with the destination features alone — within noise, as the experiment predicted); OD expected informality 36.42% → **35.96%** (sampled 35.42%; within services 46.3%, comercio 42.4%, manufactura 19.4%, gobierno/otro/agric. 12.1%); no-trip band 36.3% (unconditional) / 35.96% (conditional, shipped) / 43.2% (upper bound).

### 4.9 Two-component decomposition of informality (diagnostic) ✅ done
- **What:** ENOE `informal` split into `informal_sector` (`tue2` 5–7: informal-sector units, paid domestic work, subsistence agriculture; 27.6% of workers) and `informal_unprotected` (informal jobs in other units, i.e. no social security; 19.6%). `fit_informality_components` refits the selected specifications on each component; `evaluate_informality_components` reports held-out AUC/log loss per component; `predict_od_informality_components` applies the same marginalization to the OD. Shown in notebook 05 next to the ENOE benchmark components; no shipped output changes.
- **Result (2026-08-28):** held-out per component (metro test): `informal_sector` AUC 0.946, log loss 0.249, ECE 0.016 (observed 27.3%, predicted 27.1%); `informal_unprotected` AUC 0.811, log loss 0.306 (13.0% vs 13.2%). OD expected: informal-sector 24.6% (ENOE metro 25.5%) + unprotected 12.0% (ENOE 13.9%) = 36.6%, vs the single-model headline 35.96%. Reading: two thirds of the OD estimate is the component the shared covariates identify almost deterministically; the unprotected third is predicted with AUC 0.81 from demographics and workplace only, and is where the estimate leans most on the ENOE prior.

### 4.10 Native categorical splits in gradient boosting (C5) ✅ done
- **What:** `make_tree_preprocessor(native_categoricals=True)` ordinal-encodes the declared categories and passes `categorical_features` to `HistGradientBoostingClassifier` (random forests keep one-hot). Paired folds: sector +0.0007 ± 0.0039, informality +0.0004 ± 0.0009 — indistinguishable from one-hot. Kept as the boosting encoding (smaller design matrix, native handling of the 72-level `centralidad`), controlled by the `native_categoricals` argument of both `build_*_models`.

### 4.11 "No sabe" as its own education level — evaluated, not changed
- OD `escolaridad_raw == "No sabe"` (158 workers, 0.6%) is mapped to `no_especificado` together with item non-response. A separate `no_sabe` level would have no ENOE counterpart (ENOE has no such answer), so those workers would be marginalized over the education levels in Model A; routing them to the without-education Model B, as now, is the same treatment with a validated model. Left as is; documented in notebook 02.
